#------------------------------------ HEAD ------------------------------------
scenario = "Digit span backard task";
...
# WARNING: Again, this is a bad example of a NBSP script. If you want to reproduce,
# then do better!
...
picture {
	text {
		caption =
			"Wir bitten Sie, die Aufgabe noch einmal zu wiederholen. Dieses Mal bitten wir Sie, sich die Zahlen zu merken und dann rückwärts einzugeben.

			Beispiel: Wenn die Zahlenreihe <b>456</b> präsentiert wird, bitten wir Sie, <b>654</b> einzugeben.
...
TXT_log.open( logfile.subject() + "-digit_span_backward.txt", true );
TXT_log.print( "seq" + "	" + "resp" + "	" + "hit" + "\n" );

# Assign values in  `digit_span.txt` to the `A_digits` and randomize.
...
begin
	...
	int char_no = digits_presented.count(); # reverse the order of `digits_presented`
	string digits_reversed = "";
	loop int m = char_no until m < 1 begin   
		digits_reversed.append(digits_presented.substring( m, 1 ));
		m = m - 1;
	end;

	int digits_reversed_int = int( digits_reversed );
	response_in_int = int( response_in );
	
	if digits_reversed_int == response_in_int then
		repeat = 0;
		trial_hit = "1";
	end;
